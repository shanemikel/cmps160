...
function flat_and_gouraud_shaders(){let vert=`#version 100
    ...
    `;return{vert:vert,frag:frag};}
function render_obj(shaders,normalize_fn){return function(gl,obj,color,world,view,projection,lights){let vert=shaders.vert;let frag=shaders.frag;if(!initShaders(gl,vert,frag)){console.log('Failed to intialize shaders.');return;}
...
let light_xform=view.multiply(world);let ambiant_light;if(lights.ambiant!==null)
...
direct_light=DirectLight.neutral();let dl_dir=world.multiply(direct_light.getDirection());let dl_col=direct_light.getColor();let point_light;if(lights.point!==null)
point_light=lights.point;else
point_light=PointLight.neutral();let pl_pos=view.multiply(world).multiply(point_light.getPosition());let pl_col=point_light.getColor();let specular_light;if(lights.specular!==null)
specular_light=lights.specular;else
specular_light=SpecularLight.neutral();let sl_pow=specular_light.getPower();gl.uniformMatrix4fv(u_Model,false,world.multiply(obj.getXForm()).flatten());gl.uniformMatrix4fv(u_View,false,view.flatten());gl.uniformMatrix4fv(u_Projection,false,projection.flatten());gl.uniform3f(u_AmbiantLightColor,al_col.r,al_col.g,al_col.b);gl.uniform3f(u_DirectLight,dl_dir.x,dl_dir.y,dl_dir.z);gl.uniform3f(u_DirectLightColor,dl_col.r,dl_col.g,dl_col.b);gl.uniform3f(u_PointLight,pl_pos.x,pl_pos.y,pl_pos.z);gl.uniform3f(u_PointLightColor,pl_col.r,pl_col.g,pl_col.b);gl.uniform1f(u_SpecularPower,sl_pow);let count=obj.indices.length;obj=normalize_fn(obj);let vertices=Vector.flatten(obj.vertices);let normals=Vector.flatten(obj.normals);let buffer={vertices:gl.createBuffer(),normals:gl.createBuffer(),colors:gl.createBuffer()};if(!buffer.vertices||!buffer.normals||!buffer.colors){console.log('Failed to create the buffer objects');return;}
...
callBack(i,this.data[i]);}},remove:function(i){this.data[i]=undefined;this.count-=1;},};let WHITE=null;let BLACK=null;let RED=null;let GREEN=null;let BLUE=null;let GREY=null;let DARK_GREY=null;let LIGHT_GREY=null;let CANVAS=null;let WIDTH=null;let HEIGHT=null;let DEPTH=250;let COLOR=null;let ROTATE_X=0;let ROTATE_Y=0;let ROTATE_Z=0;let TRANSLATE_X=0;let TRANSLATE_Y=0;let TRANSLATE_Z=0;let MODELS=null;let SELECTED=null;let FOCUSED=null;let CAMERA_EYE=new Vector(0,0,350);let CAMERA_CENTER=(new Vector(0,0,0));let CAMERA_UP=new Vector(0,1,0);let AMBIANT_COLOR=(new RGBColor(1,1,1)).scale(0.15);let DIRECT_X=0;let DIRECT_Y=-100;let DIRECT_Z=-100;let DIRECT_COLOR=(new RGBColor(1,1,1)).scale(0.8);let POINT_X=250;let POINT_Y=150;let POINT_Z=250;let POINT_COLOR=(new RGBColor(1,1,1)).scale(0.8);let SPECULAR_POWER=12;let light={AMBIANT:'light-ambiant',DIRECT:'light-direct',POINT:'light-point',SPECULAR:'light-specular'};let LIGHT={ambiant:true,direct:false,point:true,specular:true};let projection={ORTHOGRAPHIC:'projection-orthographic',PERSPECTIVE:'projection-perspective'};let PROJECTION=projection.PERSPECTIVE;let shading={FLAT:'shading-flat',GOURAUD:'shading-gouraud',PHONG:'shading-phong',NORMAL:'shading-normal',DEPTH:'shading-depth',EDGE:'shading-edge',EDGE_SELECTION:'shading-edge-selection',RED_CYAN:'shading-red-cyan'};let SHADING=shading.PHONG;let shape={CYLINDER:'shape-cylinder',SPHERE:'shape-sphere'};let SHAPE=shape.SPHERE;function init(){WHITE=RGBColor.fromHex($('#g-colors').css('--white'));BLACK=RGBColor.fromHex($('#g-colors').css('--black'));RED=RGBColor.fromHex($('#g-colors').css('--red'));GREEN=RGBColor.fromHex($('#g-colors').css('--green'));BLUE=RGBColor.fromHex($('#g-colors').css('--blue'));GREY=RGBColor.fromHex($('#g-colors').css('--grey'));DARK_GREY=RGBColor.fromHex($('#g-colors').css('--dark-grey'));LIGHT_GREY=RGBColor.fromHex($('#g-colors').css('--light-grey'));WIDTH=$('#g-webgl').css('width');HEIGHT=$('#g-webgl').css('height');COLOR=RGBColor.fromHex($('#g-webgl').css('--background-color'));CANVAS=$('#webgl');CANVAS.attr('width',WIDTH);CANVAS.attr('height',HEIGHT);$('span.spacer').each(function(i,elem){let e=$(elem);e.css('width',e.attr('data-size')||e.css('--size'));});$('.tab-labels').each(function(i,elem){let e=$(elem);let def=e.attr('data-default')||e.children('label').first().attr('for');set_current_tab(elem,def);e.children('label').each(function(i,elem){let label_for=$(elem).attr('for');$(elem).click(function(ev){if($(ev.target).attr('data-disabled')!=='disabled')
set_current_tab(e,label_for)});});});setupIOSOR('file-picker');}
...
let view=Matrix.lookAt({eye:CAMERA_EYE,center:CAMERA_CENTER,up:CAMERA_UP});let width=parseInt(WIDTH);let height=parseInt(HEIGHT);let proj;switch(PROJECTION){case projection.ORTHOGRAPHIC:proj=Matrix.ortho({left:-(width/2),right:width/2,bottom:-(height/2),top:height/2,near:1,far:1000});break;case projection.PERSPECTIVE:proj=Matrix.perspective({fovy:Radians.fromDegrees(60),aspect:width/height,near:1,far:1000});break;}
...
lights.direct=null;if(LIGHT.point)
lights.point=new PointLight(new Vector(POINT_X,POINT_Y,POINT_Z),POINT_COLOR);else
lights.point=null;if(LIGHT.specular)
lights.specular=new SpecularLight(SPECULAR_POWER);else
lights.specular=null;MODELS.mapVisible(function(i,model){let color=model.getColor();switch(SHADING){case shading.FLAT:render_flat(gl,model,color,world,view,proj,lights);break;case shading.GOURAUD:render_gouraud(gl,model,color,world,view,proj,lights);break;case shading.PHONG:render_phong(gl,model,color,world,view,proj,lights);break;case shading.NORMAL:render_normal(gl,model,world,view,proj);break;case shading.DEPTH:render_depth(gl,model,color,world,view,proj,250,50);break;case shading.EDGE:render_edge(gl,model,(new RGBColor(0,0,0)),color,1/2,world,view,proj);break;case shading.EDGE_SELECTION:render_edge_selection(gl,i,model,color,world,view,proj);break;}});}
...
let view=Matrix.lookAt({eye:CAMERA_EYE,center:CAMERA_CENTER,up:CAMERA_UP});let width=parseInt(WIDTH);let height=parseInt(HEIGHT);let proj;switch(PROJECTION){case projection.ORTHOGRAPHIC:proj=Matrix.ortho({left:-(width/2),right:width/2,bottom:-(height/2),top:height/2,near:1,far:1000});break;case projection.PERSPECTIVE:proj=Matrix.perspective({fovy:Radians.fromDegrees(60),aspect:width/height,near:1,far:1000});break;}
let lights={ambiant:null,direct:null,point:null,specular:null};MODELS.mapVisible(function(i,model){let color=(new RGBColor(0,0,0)).copy().setAlpha((255-(i+1))/255);render_flat(gl,model,color,world,view,proj,lights);});}