...
let SHADER_CFG={NORMALIZE:{FLAT:1,AVERAGE:2},SHADERS:{GOURAUD:1,PHONG:2}};function renderFn(config){let normalizeFn;switch(config.normals){case SHADER_CFG.NORMALIZE.FLAT:normalizeFn=normalize_obj_flat;break;case SHADER_CFG.NORMALIZE.AVERAGE:normalizeFn=normalize_obj_average;break;}
let shaders;switch(config.shaders){case SHADER_CFG.SHADERS.GOURAUD:shaders=gouraud_shaders();break;case SHADER_CFG.SHADERS.PHONG:shaders=phong_shaders();break;}
return render_obj(shaders,normalizeFn);}
function render_model(gl,model,world,camera,projection,lights){return renderFn(model)(gl,model,model.getColor(),world,camera,projection,lights);}
let render_flat=render_obj(phong_shaders(),normalize_obj_flat);let render_gouraud=render_obj(gouraud_shaders(),normalize_obj_average);let render_phong=render_obj(phong_shaders(),normalize_obj_average);function phong_shaders(){let vert=`#version 100
...
function gouraud_shaders(){let vert=`#version 100
        attribute vec4 a_Position;
        attribute vec4 a_Normal;
        attribute vec4 a_Color;
    ...
    `;return{vert:vert,frag:frag};}
function render_obj(shaders,normalize_fn){return function(gl,model,color,world,camera,projection,lights){let vert=shaders.vert;let frag=shaders.frag;if(!initShaders(gl,vert,frag)){console.log('Failed to intialize shaders.');return;}
...
specular_light=SpecularLight.neutral();let sl_pow=specular_light.getPower();gl.uniformMatrix4fv(u_Model,false,world.multiply(model.getXForm()).flatten());gl.uniformMatrix4fv(u_View,false,camera.getView().flatten());gl.uniformMatrix4fv(u_Projection,false,projection.flatten());gl.uniform3f(u_AmbiantLightColor,al_col.r,al_col.g,al_col.b);gl.uniform3f(u_DirectLight,dl_dir.x,dl_dir.y,dl_dir.z);gl.uniform3f(u_DirectLightColor,dl_col.r,dl_col.g,dl_col.b);gl.uniform3f(u_PointLight,pl_pos.x,pl_pos.y,pl_pos.z);gl.uniform3f(u_PointLightColor,pl_col.r,pl_col.g,pl_col.b);gl.uniform1i(u_PointLightOn,pl_on);let cameraPos=camera.getEye();gl.uniform3f(u_CameraPosition,cameraPos.x,cameraPos.y,cameraPos.z);gl.uniform1f(u_SpecularPower,sl_pow);let count=model.indices.length;model=normalize_fn(model);let vertices=Vector.flatten(model.vertices);let normals=Vector.flatten(model.normals);let buffer={vertices:gl.createBuffer(),normals:gl.createBuffer(),colors:gl.createBuffer()};if(!buffer.vertices||!buffer.normals||!buffer.colors){console.log('Failed to create the buffer objects');return;}
...
res.push(new Vector(mouseXY[0],mouseXY[1],0));return res;},setColor:function(color){this.color=color.copy();},getColor:function(){return this.color.copy();},};let Cube=function(center,sideLength){this.center=center;this.side=sideLength;};Cube.prototype={getCenter:function(){return this.center.copy();},setCenter:function(center){this.center=center.copy();},getSideLength:function(){return this.side;},setSideLength:function(sideLength){this.side=sideLength;},toSquares:function(){let res=[];let delta=this.side/2;let center=this.center;res.push(center.add(new Vector(-delta,-delta,-delta)));res.push(center.add(new Vector(-delta,-delta,delta)));res.push(center.add(new Vector(-delta,delta,-delta)));res.push(center.add(new Vector(-delta,delta,delta)));res.push(center.add(new Vector(delta,-delta,-delta)));res.push(center.add(new Vector(delta,-delta,delta)));res.push(center.add(new Vector(delta,delta,-delta)));res.push(center.add(new Vector(delta,delta,delta)));return res;},toTriangles:function(){let obj={vertices:this.toSquares(),indices:[]};let range=function(start,span,stride,stop){let result=[];for(let i=start;i<stop;i+=span+stride){for(let j=0;j<span;j++)
result.push(i+j);}
return result;};let quadToTrigs1=function(quad){return[quad[0],quad[1],quad[2],quad[2],quad[1],quad[3]];};let quadToTrigs2=function(quad){return[quad[0],quad[2],quad[1],quad[1],quad[2],quad[3]];};let left=range(0,4,0,4);let right=range(4,4,0,8);let bottom=range(0,2,2,8);let top=range(2,2,2,8);let back=range(0,1,1,8);let front=range(1,1,1,8);obj.indices=obj.indices.concat(quadToTrigs2(left));obj.indices=obj.indices.concat(quadToTrigs1(right));obj.indices=obj.indices.concat(quadToTrigs1(bottom));obj.indices=obj.indices.concat(quadToTrigs2(top));obj.indices=obj.indices.concat(quadToTrigs2(back));obj.indices=obj.indices.concat(quadToTrigs1(front));return obj;},};Cube.SHADERS=SHADER_CFG.SHADERS.PHONG;Cube.NORMALS=SHADER_CFG.NORMALIZE.FLAT;let Circle=function(center,radius){this.center=center.copy();this.radius=radius;};Circle.prototype={getCenter:function(){return this.center.copy();},setCenter:function(center){this.center=center.copy();},getRadius:function(){return this.radius;},setRadius:function(radius){this.radius=radius;},toPolygon:function(sides){let sliceAngle=2*Math.PI/sides;let res=[];for(let i=0;i<sides;i++){let x,y;if(sides%2==0){x=this.radius*Math.cos(i*sliceAngle);y=this.radius*Math.sin(i*sliceAngle);}else{y=this.radius*Math.cos(i*sliceAngle);x=this.radius*Math.sin(i*sliceAngle);}
...
let bottom_vertex=new Vector(this.center.x,this.center.y,this.center.z-this.radius);let bottom_polygon=polygons[polygons.length-1];obj.vertices=obj.vertices.concat(bottom_polygon);obj.vertices.push(bottom_vertex);return obj;},};Sphere.SHADERS=SHADER_CFG.SHADERS.PHONG;Sphere.NORMALS=SHADER_CFG.NORMALIZE.AVERAGE;let Cylinder=function(end1,end2,radius,color){this.end1=end1.copy();this.end2=end2.copy();end1.z=0;end2.z=0;this.radius=radius;if(color!==undefined)
...
callBack(i,this.data[i]);}},remove:function(i){this.data[i]=undefined;this.count-=1;},};let WHITE=null;let BLACK=null;let RED=null;let GREEN=null;let BLUE=null;let GREY=null;let DARK_GREY=null;let LIGHT_GREY=null;let CANVAS=null;let WIDTH=null;let HEIGHT=null;let DEPTH=250;let COLOR=null;let ROTATE_X=0;let ROTATE_Y=0;let ROTATE_Z=0;let TRANSLATE_X=0;let TRANSLATE_Y=0;let TRANSLATE_Z=0;let MODELS=null;let SELECTED=null;let FOCUSED=null;let CAMERA;{let EYE=new Vector(0,0,400);let FRONT=(new Vector(0,0,0)).sub(EYE);let UP=new Vector(0,1,0);CAMERA=new Camera(EYE,FRONT,UP);}
let AMBIANT_COLOR=(new RGBColor(1,1,1)).scale(0.15);let DIRECT_X=0;let DIRECT_Y=-100;let DIRECT_Z=-100;let DIRECT_COLOR=(new RGBColor(1,1,1)).scale(0.6);let SHOW_POINT=true;let POINT_X=150;let POINT_Y=10;let POINT_Z=80;let POINT_COLOR=(new RGBColor(1,1,1));let SPECULAR_POWER=32;let TABS=[];let TAB_COUNTER=0;let light={AMBIANT:'light-ambiant',DIRECT:'light-direct',POINT:'light-point',SPECULAR:'light-specular'};let LIGHT={ambiant:false,direct:true,point:false,specular:true};let PERSPECTIVE_FOVY=60;let projection={ORTHOGRAPHIC:'projection-orthographic',PERSPECTIVE:'projection-perspective'};let PROJECTION=projection.PERSPECTIVE;let shading={MODEL:'shading-model',FLAT:'shading-flat',GOURAUD:'shading-gouraud',PHONG:'shading-phong',NORMAL:'shading-normal',DEPTH:'shading-depth',EDGE:'shading-edge',EDGE_SELECTION:'shading-edge-selection',RED_CYAN:'shading-red-cyan'};let SHADING=shading.MODEL;let shape={CYLINDER:'shape-cylinder',SPHERE:'shape-sphere'};let SHAPE=shape.SPHERE;let KEYS={y:89,u:85,g:71,h:72,i:73,o:79,j:74,k:75,LEFT:37,RIGHT:39,UP:38,DOWN:40};function init(){WHITE=RGBColor.fromHex($('#g-colors').css('--white'));BLACK=RGBColor.fromHex($('#g-colors').css('--black'));RED=RGBColor.fromHex($('#g-colors').css('--red'));GREEN=RGBColor.fromHex($('#g-colors').css('--green'));BLUE=RGBColor.fromHex($('#g-colors').css('--blue'));GREY=RGBColor.fromHex($('#g-colors').css('--grey'));DARK_GREY=RGBColor.fromHex($('#g-colors').css('--dark-grey'));LIGHT_GREY=RGBColor.fromHex($('#g-colors').css('--light-grey'));WIDTH=$('#g-webgl').css('width');HEIGHT=$('#g-webgl').css('height');COLOR=RGBColor.fromHex($('#g-webgl').css('--background-color'));CANVAS=$('#webgl');CANVAS.attr('width',WIDTH);CANVAS.attr('height',HEIGHT);$('span.spacer').each(function(i,elem){let e=$(elem);e.css('width',e.attr('data-size')||e.css('--size'));});$('.tab-labels').each(function(i,elem){let e=$(elem);let id=TAB_COUNTER++;e.attr('data-id',id);TABS[id]=[];let def=e.attr('data-default')||e.children('label').first().attr('for');set_current_tab(elem,def);e.children('label').each(function(i,elem){if($(elem).attr('data-disabled')!=='disabled')
...
lights.specular=null;if(SHOW_POINT){let point=new Sphere(lights.point.getPosition(),15);point=point.toTriangles(50,50);let pointLight=new Model('Point Light',point.indices,point.vertices,{color:lights.point.getColor()});render_model(gl,pointLight,world,CAMERA,proj,{ambiant:new AmbiantLight((new RGBColor(1,1,1))),direct:null,point:null,specular:null});}
MODELS.mapVisible(function(i,model){let color=model.getColor();switch(SHADING){case shading.MODEL:render_model(gl,model,world,CAMERA,proj,lights);break;case shading.FLAT:render_flat(gl,model,color,world,CAMERA,proj,lights);break;case shading.GOURAUD:render_gouraud(gl,model,color,world,CAMERA,proj,lights);break;case shading.PHONG:render_phong(gl,model,color,world,CAMERA,proj,lights);break;case shading.NORMAL:render_normal(gl,model,world,view,proj);break;case shading.DEPTH:render_depth(gl,model,color,world,view,proj,250,50);break;case shading.EDGE:render_edge(gl,model,(new RGBColor(0,0,0)),color,1/2,world,view,proj);break;case shading.EDGE_SELECTION:render_edge_selection(gl,i,model,color,world,view,proj);break;}});}
